...
- [ ] `examples/` 的 MODE-A/B 示例 `build_and_check` 因 `已核验` 计数（含正文里"标 已核验 ✓"的散文提及 + 无 x-verify 工件的装饰徽章）而 FAIL——**先于本轮、与本轮无关**；待单独一轮把示例的徽章补 x-verify 或降级 `未自动核验`，或让 `check_verified_badges` 只数徽章不数散文。
